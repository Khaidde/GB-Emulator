...
void CPU::handle_interrupts() {
    ...
    if (interrupts) {
        if (halted) cycles++;  // Extra cycle if cpu is in halt mode
        ...
        if (ime) {
            for (int i = 0; i < 5; i++) {
                if (interrupts & (1 << i)) {
                    ...
                    cycles++;  // Interrupt handling takes total of 5 cycles (+1 if halted)
                    break;
                }
            }
        }
...
void CPU::push(u16 reg) {
    cycles++;  // Stack push takes an extra cycle
    write(--SP, reg >> 8);
    write(--SP, reg & 0xFF);
}
...
void CPU::opCB() {
    ...
    if (cycles != test_cb_cycles[op]) {
        printf("CB: %d != expected %d cycles (%02x)\n", cycles, test_cb_cycles[op], op);
        debugger->pause_exec();
    }
}  // CB-prefix
